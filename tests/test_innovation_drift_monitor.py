...
def test_a_nominal_run_with_no_planted_drift_rarely_trips_the_monitor() -> None:
    report = _drift_report(with_drift=False, seed=0)
    flagged_fraction = report.drift_detected.mean()
    assert flagged_fraction < 0.2


def test_a_planted_drift_is_detected_after_the_change_point() -> None:
...
def test_the_planted_drift_run_flags_far_more_windows_than_the_nominal_run() -> None:
