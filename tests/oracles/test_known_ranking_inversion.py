...
def test_material_a_is_weaker_at_the_thick_process_zone() -> None:
    oracle = KnownRankingInversionOracle()
    thick, _thin = oracle.truth()
    assert oracle.failure_probability("A", thick) > oracle.failure_probability("B", thick)


def test_material_b_is_weaker_at_the_thin_process_zone() -> None:
    oracle = KnownRankingInversionOracle()
    _thick, thin = oracle.truth()
    assert oracle.failure_probability("B", thin) > oracle.failure_probability("A", thin)


def test_the_ranking_genuinely_inverts_between_the_two_thicknesses() -> None:
...
def test_the_inversion_is_driven_by_dimensional_reduction_not_by_severity_alone() -> None:
