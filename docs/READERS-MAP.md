...
**All three stages are executed; `docs/ARITY-REDESIGN-RECORD.md` is the closeout** — read it for
what closed, what did not, and the three outstanding pressures that keep the item count unstable.
It also records one thing the closeout could **not** assess: a fourth pressure raised in the Stage 3
authorisation (an "E-63" concerning a kesterite domain) that exists nowhere in this repository or
its history, so the count-stability statement is made over three pressures rather than four.

**Stage-by-stage:** Stage 1 (ADR-070) added the scope, lineage
...
(`audit/baselines/redesign-items8.json`; see `audit/BASELINES.md`). Stage 3 (ADR-072) made the
`observe()` bound checkable at record time (E-59) and moved **no** observation at all. Read the
brief for the plan and ADR-070/071/072 for what was built, including **four** places where the
brief's cost model was wrong — all in the same direction, itemised in the closeout record.
