...
| S-11.4 | §11.4 | Four interface-only sketches (M10.1, docs/ROADMAP.md) | **PASS-D** — 1 of 4 written | ADR-038 (docs/DECISIONS.md): `src/omi_domains/sketches/<name>.py`, prose in `docs/SKETCHES.md` | `src/omi_domains/sketches/device_yield.py` (device yield only; layer-wise additive processing, crystallisation and formulation, and one deliberately-awkward fourth remain unwritten) | — (no synthetic oracle; the cross-domain diff against flagship/contrast is the check, same as C-4/S-11) | `tests/test_sketches.py` (device yield only) | — (own status PASS-D; a provider, not a dependent, row — depends on nothing, and C-6.2's fifth-slot open problem depends on the fourth, not-yet-written sketch, not on this one) |
