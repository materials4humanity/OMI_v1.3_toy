...
| S-11.4 | §11.4 | Four interface-only sketches (M10.1, docs/ROADMAP.md) | **PASS-D** — 2 of 4 written | ADR-038 (docs/DECISIONS.md): `src/omi_domains/sketches/<name>.py`, prose in `docs/SKETCHES.md` | `src/omi_domains/sketches/device_yield.py`, `src/omi_domains/sketches/layerwise_additive.py` (crystallisation and formulation and one deliberately-awkward fourth remain unwritten) | — (no synthetic oracle; the cross-domain diff against flagship/contrast is the check, same as C-4/S-11) | `tests/test_sketches.py` (device yield, layer-wise additive) | **C-2.5** (PASS-C) — the layer-wise additive sketch's item 1 (state schema) is a forced Tier I approximation for exactly the reason S-4.4/S-4.6 already name: no body-indexed state representation exists to declare the domain's actual, growing-body state faithfully. New finding, not merely a repeat of S-4.4's: this blocks the *declaration* itself (item 1), not only downstream Class-B validation — `docs/V1.4-EDITS.md` E-21 |
