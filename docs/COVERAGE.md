...
| C-6.1 | §6.1 | Six falsification criteria | **PASS-D** (thresholds) | criteria as tests; thresholds via ADR | `src/omi/sufficiency.py` (`BlockingTerm.BIAS`, `diagnose_blocking_term`) implements **one of six** criteria (criterion 1, bias-blocked exhaustion); the other five are not operationalised as checks anywhere in the repo | `tests/test_sufficiency_augmentation_loop.py` (not under `tests/oracles/`, despite testing an estimator against a constructed scenario — a naming/placement inconsistency worth fixing) | — | **C-3.7** (PASS-C, criterion 2: closure defect), **C-3.9b** (PASS-B, criterion 6: error-control dichotomy), **S-9.3/S-7.1** (PASS-C/PASS-B, criterion 4: baselines + inverse-design hit rate) — of the six falsification criteria (Core §6.1), three (2, 4, 6) cannot be checked at all without resolving other PASS rows first; only criteria 1 and 5 (sufficiency/state-selection) are checkable against current SPEC machinery, and criterion 3 (rollout error between erasures) needs no further PASS resolution |
...
| S-9.3 | §9.3 | Baselines; grouped splits; prospective validation | SPEC / **PASS-C** (go-no-go table) | `conformance.py` | partial — grouped splits (`src/omi/learning.py::grouped_train_test_split`, see S-5.3) implemented; a distinct baseline-comparison or prospective-validation function is not implemented, only a completeness-gate requirement name in `src/omi/conformance.py` | — | `tests/test_learning_oracle.py` (contrast, grouped splits only) | **S-7.1** (PASS-B) — chased this session: `ReachabilityCertificate`'s `Φ(s)=w·s` is architecturally linear-only (ADR-031), so a domain whose only declared item-6 invariant is nonlinear could never construct one with this repo's `inverse.py`. **Not found to be a defect in the conformance regime itself**: `conformance.py`'s OMI-2 check is a bare presence check (ADR-028), and Spec §7.1's own hierarchy anticipates weaker, unsound-but-present certificates (forward sampling) that would satisfy it — the blocker is `inverse.py`'s scope (already tracked at S-7.1), not the regime's design. Neither `flagship` nor `contrast` currently tests this path at all, and both domains' own declared invariants (mass/charge conservation, single-slot monotone accumulation) are linear-friendly, so the blocked case is not empirically demonstrated, only shown logically possible. No new E-entry proposed. |
