...
| C-4 | §4 | Instantiation interface — **seven items as v1.3 issues them, eight as this repository declares them** (ADR-071 split item 1 into 1a state schema / 1b declared parameters, per `docs/V1.4-EDITS.md` E-46; `omi.interface.INTERFACE_ITEMS` is the single authority for the count, the charters and the Core §7.2 row mapping, so no document or test should write the number as a literal — E-01) | SPEC | `omi_domains/*/interface.py` — ADR-034 (docs/DECISIONS.md) supersedes ADR-003's pinning test: Core §7.2's seven rows name six distinct Core §4 items, not seven, and `interface.diff()` now also reports a structural (kind-based) comparison for item 6's invariants alongside the literal one. Item 4b (per-Class-B-readout driver field/defect population/physics map Ψ) is now filled for the flagship's `bend_angle` (Phase 2.2, ADR-035) — the first domain to fill 4b at all; the contrast's `dendrite_risk` (Type-0/Class-B) still does not declare it | `src/omi/interface.py`, `src/omi_domains/flagship/interface.py`, `src/omi_domains/contrast/interface.py` | — (no synthetic oracle; the cross-domain diff is the check) | `tests/test_interface_diff.py` (flagship, contrast) — **rebuilt at ADR-071 so Core §7.2's row-to-item mapping is derived from `INTERFACE_ITEMS` rather than pinned by hand, which is E-01 converted from a presentation defect into a standing check; it reported the changed count (seven published rows covering six of eight declared items, with 1b and 6 row-less) on its first run**; `tests/test_flagship_classb_bend.py::test_flagship_declares_bend_angle_with_item_4b_filled` (flagship, item 4b specifically); `tests/test_sketches.py` (all four sketches' item-1b verdicts, re-derived per CLAUDE.md invariant 11) | — (the interface-declaration/diff claim itself needs no PASS machinery; the OMI-2-level dependency lives at S-9.3, not here) |
...
| S-2.2 | §2.2 | Hard structural constraints | SPEC | `constraints.py` — positivity, simplex, monotonicity, conservation implemented; symmetry left to domains (ADR-030), and as of ADR-070 a domain may now *declare* the group and the components it acts on (`omi.proposed.decision.SymmetryGroupAction`, `docs/V1.4-EDITS.md` E-30) — but `omi.state.Metric` still does not quotient by it, so the equivariant architecture §2.2 requires is undeclared-input no longer but is still unbuilt; thermodynamic admissibility (GENERIC/port-Hamiltonian) not implemented, scope limitation per ADR-030 | `src/omi/constraints.py`, `src/omi/proposed/decision.py` | `tests/test_constraints.py` (off-manifold, adversarial-scale checks; not under `tests/oracles/`); **proposed sixth hard-constraint category (M11.2-M11.3, beyond Spec):** `tests/oracles/known_envelope.py`, `tests/oracles/test_known_envelope.py` — declared constitutive forms with a validated range known by construction, `src/omi/proposed/constitutive.py`, per ADR-043 and `docs/V1.4-EDITS.md` E-32; exercised on real physics by `src/omi_domains/flagship_constitutive/forms.py` (five canonical forms, three two-sided windows and one one-sided, one approximate edge) with `tests/test_flagship_constitutive.py`. **Not an implementation of §2.2**: §2.2 names five categories and none is a constitutive form, so this is a proposed *addition* carrying evidence, reachable only through `omi.proposed` and stamped `SpecificationVersion.PROPOSED_CONSTITUTIVE_EXTENSION`; the symmetry-declaration repair is the same kind of proposed addition, stamped `PROPOSED_DECISION_EXTENSION`, pinned by `tests/test_arity_stage1_additive.py` | — (confirmed by grep: `src/omi/learning.py` never imports `src/omi/constraints.py` — the constraint layers are implemented but not architecturally wired into the one learned operator the repo has, which is in tension with CLAUDE.md §5 invariant 5's "hard constraints are architecture, never loss penalties") | — (thermodynamic admissibility is an unimplemented sub-item of this SPEC section, ADR-030, not a distinct PASS row) |
...
| S-3.2 | §3.2 | Erasure truncates the Gramian; Prop 3.2, Cor 3.3 | SPEC — **corrected beyond Spec, ADR-068**: `erasure.py` now reports completeness as **two independent quantities** (effective rank at a declared tolerance, and gain on the surviving subspace in the same declared metric), because Core §3.4 conjoins them and a real operator separates them — calcination is rank 1 of 7 with a surviving-subspace gain of 14.44 (`docs/V1.4-EDITS.md` **E-56**). `ErasureCompleteness.__bool__` raises rather than synthesising a combined score. And `condition_a_claim` **refuses** Core §3.9's condition (a) when no sufficiency deficit for the declared state is supplied, because a state-space erasure cannot bound error from a variable outside the state space (**E-57**); the claim object is not yet wired into `conformance.py`, deliberately, pending the arity redesign. Neither is an implementation of §3.2 — both are corrections to what §3.4/§3.9 license, carrying evidence | `observability.py`, `erasure.py` | `src/omi/observability.py::compute_gramian` (Gramian half); `src/omi/erasure.py` (erasure half) | `tests/oracles/test_erasure_truncates_gramian.py`, `tests/test_erasure_two_quantities.py` (E-56/E-57's repairs, 6 checks) | — (no domain test combines a declared erasure with a downstream Gramian; `tests/test_domain_triage.py` computes Gramians on both domains but flagship's declared erasure precedes the sensors it uses, not measured jointly, and the erasure-truncation *effect* is checked separately and only qualitatively in `tests/test_error_compounding.py`) | — |
...
hypothesis, what the measured result was. Proposed wording for each
...
>
> **Proposed wording:** none — recorded as a deliberate confirmation
